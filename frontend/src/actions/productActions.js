...
export const analyseProduct = () => async(dispatch, getState) => {
    try{
        dispatch({type: PRODUCT_ANALYSIS_REQUEST});
        ...
        const {data} = await axios.get(`/api/products/analysis`, config);

        dispatch({type: PRODUCT_ANALYSIS_SUCCESS, payload: data});

    } catch (err) {
        dispatch ({ type: PRODUCT_ANALYSIS_FAIL,
            payload: err.response && err.response.data.message
            ? err.response.data.message
            : err.message,
        })
